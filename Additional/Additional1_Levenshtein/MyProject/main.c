...
void putLine(char words[WORDS_PER_LINE+1][WORDS_LEN]) {
    int i=0, j=0;
    for (i=0 ; i<WORDS_PER_LINE+1 && words[i][0]!='\0' ; i++) {
        if (i=0) {
            for (j=0 ; j<WORDS_LEN && words[i][j]!='\0' ; j++) printf ("%c", words[i][j]);
            putchar(' ');
        }
        else {
            if (i!=1) putchar('/');
...
int main() {
    ...
    while (!feof(wordsFile)) {
        ...
        for (i=1 ; words[i][0]!='\0' || i!=5 ; i++) {
            ...
            char Nearest[WORDS_LEN];
            while (!feof(vocabularyFile) || min!=0) {
                int EditDist=0;
                /* Read Vocabulary one by one */
                char wordInVocabulary[WORDS_LEN];
                fgets(wordInVocabulary, WORDS_LEN, vocabularyFile);
                for (i=0 ; wordInVocabulary[i]!='\n' && i<WORDS_LEN ; i++); /* count word in vocabulary's length */
